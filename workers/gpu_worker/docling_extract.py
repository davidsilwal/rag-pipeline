...
def docling_available() -> bool:
    """True when the ``docling`` package can be imported."""
    try:
...
def _provenance(meta: Any) -> tuple[int | None, list | None]:
    ...
    ``(page_no, BoundingBox)`` tuples. We surface the first page and all
    bounding boxes so a retrieved unit can highlight the exact page region.
    """
    page: int | None = None
    bboxes: list = []
    for item in getattr(meta, "doc_items", None) or []:
        for prov in getattr(item, "prov", None) or []:
            try:
                page_no, bbox = prov  # tuple (page_no, BoundingBox)
            except (TypeError, ValueError):
                continue
            if page is None and isinstance(page_no, int):
                page = page_no
            bboxes.append(_bbox_to_dict(bbox))
